...
describe "purchases/show.html.haml" do
  before(:each) do
    ...
    @purchase = stub_model(Purchase, :person => @person, :purchased_at => Time.now, :total => 123, :workflow_state => "Ny", :budget_post => stub_model(BudgetPost), :updated_at => Time.now, :last_updated_by => @person)
