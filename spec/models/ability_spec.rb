...
describe Ability do
  ...
  describe 'ordinary user' do
    ...
    it 'should not be able to pay/bookkeep its own purchases' do
      expect(@ability).not_to be_able_to(:pay, @user.purchases.new(workflow_state: 'confirmed'))
      expect(@ability).not_to be_able_to(:keep, @user.purchases.new(workflow_state: 'confirmed'))
    end
