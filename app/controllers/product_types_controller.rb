class ProductTypesController < ApplicationController
  ...
  def create
    ...
    if @product_type.save
      redirect_to(@product_type,
                  notice: I18n.t("notices.product_type.success.created"))
    else
      render action: "new"
    end
  end
  ...
  def update
    if @product_type.update_attributes(params[:product_type])
      redirect_to(@product_type,
                  notice: I18n.t("notices.product_type.success.updated"))
    else
      render action: "edit"
    end
  end
  ...
  def destroy
    unless @product_type.destroy
      flash[:error] =
        I18n.t("activerecord.errors.models.product_type.cannot_be_removed")
    end
